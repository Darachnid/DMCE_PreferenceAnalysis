...
for (i in i:3) {
  ...
  swim <- swim |>
    ...
    mutate(
      ...
      rollSE = rollmean(se, k = k, fill = NA, align = "center"),
      rollRange = rollmean(range, k = k, fill = NA, align = "center"),
      rate_of_change = (rollRange - lag(rollRange, n = window)) / (Min - lag(Min, n = window))
    )
  ...
  joinedY <- joinedT |>
    mutate(Min = round(Min * 2) / 2)  |>
    select(Group, ES, Participant, Min, Value) |>
    left_join(allA, by = c("ES", "Min", "Group")) |>
    select(Group, ES, Participant, Min, Value, rollMean) |>
    mutate("RSS" = sqrt((rollMean - Value)^2)) |>
    filter(floor(Min) == Min)

  boxes[[i]] <- ggplot(joinedY, aes(
    x = Min
  )) +
    geom_step(
      size = 1,
      position = "identity",
      aes(y = Value, color = Participant),
      alpha = 0.3
    ) +
    geom_line(aes(y = rollMean),
              color = main,
              size = 2) +
    scale_color_manual(values = rep("grey60", 6)) +
    labs(
      title = paste0("Workshop ", group_num),
      x = "Duration of Deliberation (min)",
      y = "Importance"
  ...
  ggsave(paste0(here::here("out/Group_"), group_num, "_bags.png"), width = 400, height = 200, units = "mm", bg = "black")
  ...
  woo <- joinedY |>
    group_by(Group, ES, Min) |>
    reframe(rollMeans = mean(rollMean),
            RSS = sum(sqrt((rollMean - Value)^2))) |>
    mutate(rate_of_change = (RSS - lag(RSS, n = window)) / (Min - lag(Min, n = window)))
    
  converge[[i]] 
  woo |>
    ggplot(aes(x = Min, y = RSS)) +
    geom_line() +
    facet_wrap(~ES, nrow = 4)
  woo |>
    ggplot(aes(x = Min, y = rate_of_change)) +
    geom_line() +
    facet_wrap(~ES, nrow = 4)
